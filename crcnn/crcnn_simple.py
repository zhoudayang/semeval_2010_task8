...
def calculate_loss(score_list, plus_index):
    plus_score = score_list[plus_index]
    ...
    return tf.log(1.0 + tf.exp(2.0 * (2.5 - plus_score))) + tf.log(1.0 + tf.exp(2.0 * (0.5 + minus_score)))
...
def zy_loss(y_true, y_pred):
    ...
    losses = tf.map_fn(lambda i: calculate_loss(y_pred[i, :], label[i]), tf.range(batch_size),
                       dtype=tf.float32)
...
if __name__ == "__main__":
    ...
    # 这里算出来的是分数
    output = Dense(RELATION_COUNT, use_bias=False, activation=None)(output)
