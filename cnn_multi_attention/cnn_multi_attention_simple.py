...
class f1_calculator(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        logs = logs or {}
        predict_result = self.model.predict(x=[self.index, self.e1_pos, self.e2_pos, self.relation_arr])
...
if __name__ == "__main__":
    ...
    model = keras.Model(inputs=[index_input, relative_e1_index_input, relative_e2_index_input, relation_input],
                        outputs=[output])
    ...
    model.fit(x=[train_index, train_relative_e1_pos, train_relative_e2_pos, train_relation_input_arr], y=[train_labels],
