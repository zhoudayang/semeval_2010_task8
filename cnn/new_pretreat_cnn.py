...
from pretreat.new_pretreat import FIXED_SIZE
from pretreat.new_pretreat import POSITION_EMBEDDING_LENGTH
...
class f1_calculator(Callback):
    ...
    def save_best(self, f1):
        with open("/home/zy/git/zy_paper/cnn/new/README", "r") as file:
            best_f1 = file.readline()
            best_f1 = float(best_f1)
        if f1 > best_f1:
            os.system("rm /home/zy/git/zy_paper/cnn/new/README")
            with open("/home/zy/git/zy_paper/cnn/new/README", "w") as file:
                file.write(str(f1))
            self.model.save("/home/zy/git/zy_paper/cnn/new/simple_cnn.model")
...
if __name__ == "__main__":
    ...
    pos1_embedding = Embedding(input_dim=POSITION_EMBEDDING_LENGTH, output_dim=POS_EMBEDDING_DIM,
                               input_length=FIXED_SIZE,
                               trainable=True) \
        (pos1_input)

    pos2_embedding = Embedding(input_dim=POSITION_EMBEDDING_LENGTH, output_dim=POS_EMBEDDING_DIM,
                               input_length=FIXED_SIZE,
                               trainable=True) \
        (pos2_input)
    ...
    # print Conv1D(filters=150, kernel_size=2, strides=1, padding="same").compute_output_shape([None, FIXED_SIZE, EMBEDDING_DIM + 2 * POS_EMBEDDING_DIM])
