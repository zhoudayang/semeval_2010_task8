...
sys.path.append("..")
from pretreat1.common_pretreat import RELATION_COUNT
...
if __name__ == "__main__":
    ...
    FIXED_SIZE = train_relative_e1_pos.shape[1]
    ...
    pos1_embedding = Embedding(input_dim=2 * FIXED_SIZE, output_dim=POS_EMBEDDING_DIM, input_length=FIXED_SIZE,
                               trainable=True) \
        (pos1_input)

    pos2_embedding = Embedding(input_dim=2 * FIXED_SIZE, output_dim=POS_EMBEDDING_DIM, input_length=FIXED_SIZE,
                               trainable=True) \
        (pos2_input)
