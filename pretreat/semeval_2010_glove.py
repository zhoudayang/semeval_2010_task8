...
if __name__ == "__main__":
    ...
    df["relative_e1_pos_with_indicator"] = df["e1_pos_with_indicator"].map(
        lambda x: map(lambda y: x - y, range(FIXED_SIZE)))
    ...
    for word in all_words:
        if len(word.split(" ")) > 1:
            ...
            for one_word in word_list:
                if one_word.lower() in glove_lower_words:
                    vec_list.append(glove_vec[glove_lower_words[one_word.lower()]])
            if len(vec_list) == 0:
                vec_list.append(glove_vec[glove_words["UNK"]])
            ...
            if word not in my_words:
                ...
                index += 1
        elif word in glove_words:
            my_vec.append(glove_vec[glove_words[word]])
            my_words[word] = index
            my_lower_words[word.lower()] = index
            index += 1
        else:
            word = word.lower()
            if word in glove_lower_words:
                my_vec.append(glove_vec[glove_lower_words[word]])
    ...
    np.save("train_glove_with_indicator_index.npy", train_wiki_with_indicator_index)
    np.save("train_glove_without_indicator_index.npy", train_wiki_without_indicator_index)
    np.save("test_glove_with_indicator_index.npy", test_wiki_with_indicator_index)
    np.save("test_glove_without_indicator_index.npy", test_wiki_without_indicator_index)
