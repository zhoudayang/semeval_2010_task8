...
from keras.layers import Dense, Conv1D, Dropout, Input, concatenate, MaxPooling1D, Flatten, LSTM, Bidirectional, \
    RepeatVector, Reshape, TimeDistributed, Activation, MaxPool1D, Lambda, BatchNormalization, AveragePooling1D, \
    GlobalMaxPool1D
...
if __name__ == "__main__":
    ...
    embedding_dim = EMBEDDING_DIM + 2 * POS_EMBEDDING_DIM
    # embedding_output = LSTM(units=embedding_dim, return_sequences=True, activation="tanh")(embedding_output)
    # embedding_output = AveragePooling1D(pool_size=FIXED_SIZE)(embedding_output) # 这里的平均没有意义
    embedding_output = LSTM(units=embedding_dim, activation="tanh", return_sequences=True)(embedding_output)
